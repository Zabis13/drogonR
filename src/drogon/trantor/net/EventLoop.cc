...
void EventLoop::wakeupRead()
{
    ...
    if (ret < 0)
    {
        // During shutdown the wakeup fd can race with the loop tearing down
        // (EAGAIN on a non-blocking eventfd, EBADF if already closed). Don't
        // FATAL on those — only complain if we're still meant to be running.
