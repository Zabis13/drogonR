...
{
#if defined(DROGONR_NO_LIBUUID) && !defined(_WIN32)
    // drogonR fallback: portable UUIDv4 via <random>, no libuuid required.
    // Session IDs do not need a cryptographically strong UUID.
    ...
    return createUuidString(reinterpret_cast<const char *>(bytes), 16, lowercase);
#elif USE_OSSP_UUID
    uuid_t *uuid;
    uuid_create(&uuid);
